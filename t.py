...
# ==========================================
# Put your personal Gmail here so the bot shares the database with you!
...
# Replace your old CREDS block with this:
def get_user_creds():
    creds = None
    # Token.pickle stores your login so you don't have to log in every time
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            # This uses the new file you downloaded
...
# Initialize services using User credentials instead of Service Account
...
def setup_google_workspace():
    """Automatically creates Drive folders and Sheets if they don't exist."""
    if os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'r') as f:
            return json.load(f)
            
    print("🚀 First run detected! Setting up Google Workspace...")
    ...
    # Share Drive Folder (Public read so images load on the web)
    ...
    # Share Spreadsheet with Admin (so you can view it)
    if ADMIN_EMAIL and "@" in ADMIN_EMAIL:
        sheet.share(ADMIN_EMAIL, perm_type='user', role='editor')

    # Setup Users Worksheet
    users_ws = sheet.get_worksheet(0)
    users_ws.update_title('Users')
    users_ws.append_row(['id', 'username', 'password', 'display_name', 'avatar_color', 'created_at', 'last_seen'])
    
    # Setup Messages Worksheet
    messages_ws = sheet.add_worksheet(title='Messages', rows=1000, cols=11)
    messages_ws.append_row(['id', 'sender', 'recipient', 'content', 'file_url', 'file_type', 'file_name', 'is_file', 'timestamp', 'is_read', 'reaction'])
    ...
    print(f"✅ Setup Complete! Sheet ID: {sheet_id}")
    return config_data
...
# Connect to the initialized sheets
...
# 2. CACHE & ENCRYPTION
# ==========================================
# Caching reduces Google API calls (crucial for staying within rate limits)
user_cache = {}
connected_users = {} # Maps username -> socket SID
    ...
    try:
        ...
        for row in records:
            if str(row.get('id')) == str(user_id):
                user = User(row['id'], row['username'], row['password'], row['display_name'], row['avatar_color'], row['created_at'], row['last_seen'])
    ...
    if request.method == 'POST':
        ...
        if action == 'register':
            ...
            new_user_data = [
                ...
                username, random.choice(colors), datetime.utcnow().isoformat(), datetime.utcnow().isoformat()
            ]
            ...
            if existing_user and check_password_hash(existing_user['password'], password):
                user = User(existing_user['id'], existing_user['username'], existing_user['password'], existing_user['display_name'], existing_user['avatar_color'], existing_user['created_at'], existing_user['last_seen'])
...
def upload_file():
    ...
    try:
        ...
        file_metadata = {
            'name': filename, 
            'parents': [DRIVE_FOLDER_ID] # This places it in your NEXUS_UPLOADS
        }
        ...
        # 3. Clean up the local temp file immediately (Essential for Windows)
        media._fd.close() 
        if os.path.exists(filepath):
            os.remove(filepath)

        # 4. Make it Publicly Viewable (So it shows in the Chat UI)
        drive_service.permissions().create(
            fileId=file_id,
            body={'type': 'anyone', 'role': 'reader'}
        ).execute()

        # 5. Generate the Direct Image Link
        # This link works directly in <img> tags without CORS issues
        ...
        print(f"❌ DRIVE UPLOAD ERROR: {e}") # Check your terminal for this!
...
def unread_counts():
    try:
        ...
        counts = {}
        for row in records:
            if row.get('recipient') == current_user.username and str(row.get('is_read', 'FALSE')).upper() == 'FALSE':
...
# ==========================================
# 5. SOCKET.IO (CHAT & WEBRTC CALLING)
# ==========================================
@socketio.on('connect')
def handle_connect():
    if current_user.is_authenticated:
